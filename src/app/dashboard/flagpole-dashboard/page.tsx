    ...
    async function fetchStats() {
      ...
      try {
        const res = await fetch(
          `/api/admin/flagpole-dashboard?date=${selectedDate}&range=${trendRange}&_t=${Date.now()}`,
        );
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
                  ...
                  recentCheckIns.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 group/row">
                      <div className="relative">
                        ...
                        <div
                          className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-zinc-900 ${item.status === "Late" ? "bg-amber-500" : "bg-emerald-500"}`}
                        />
                      </div>
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={departmentStats} layout="vertical">
                    ...
                    />
                    <Bar dataKey="count" fill="#6366f1" radius={[0, 8, 8, 0]} barSize={12} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
        ...
        <div className="pt-16 pb-8 text-center border-t border-slate-100 dark:border-zinc-900">
          <p className="text-[10px] text-slate-300 dark:text-zinc-700 font-black uppercase tracking-[0.5em] leading-loose">
            ระบบวิเคราะห์การเข้าร่วมกิจกรรมหน้าเสาธง วิทยาลัยเกษตรและเทคโนโลยีศรีสะเกษ <br />© 2026
            SAKCAT DATACENTER DEPARTMENT
          </p>
        </div>
      </div>
    </div>
